...
export async function fetchDni(dni: string) {
  if (!useApisPeruForDni && factilizaToken) {
    try {
      const url = `${factilizaBase.replace(/\/+$/, '')}/dni/info/${dni}`;
      ...
      const ubigeoDir =
        (d.ubigeo_reniec || d.ubigeo_sunat || (Array.isArray(d.ubigeo) ? d.ubigeo.join('-') : d.ubigeo) || '').toString();
      return {
        dni: d.numero || dni,
        nombres,
        apPat,
