...
export const BackendService = {
  ...
  async createProduct(payload: Partial<Product>): Promise<Product> {
    const res = await api.post('/products', {
      ...
      supplier_id: payload.supplierId ? Number(payload.supplierId) : null,
      block_number: payload.blockNumber ? Number(payload.blockNumber) : null,
  ...
  async updateProduct(id: string, payload: Partial<Product>): Promise<Product> {
    const res = await api.put(`/products/${id}`, {
      ...
      notary_cost: payload.notaryCost,
      total_cost: payload.totalCost,
      intermediary_id: payload.intermediaryId ? Number(payload.intermediaryId) : undefined,
      supplier_id: payload.supplierId ? Number(payload.supplierId) : undefined,
      block_number: payload.blockNumber ? Number(payload.blockNumber) : undefined,
      transfer_base: payload.transferBase,
      transfer_igv: payload.transferIgv,
      transfer_total: payload.transferTotal,
      transfer_doc_type: payload.transferDocType,
